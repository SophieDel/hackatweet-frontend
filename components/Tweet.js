...
function Tweet(props) {
    ...
    // const timelaps = "Il y a 2 heures"

    const tweet = props.contentInArray.map((word, i) => {
        const isHashtag = props.hashtags.includes(word) ? true : false
        return (<span className={styles.spanTweet} key={i} style={isHashtag ? {"color": "#013F88"} : {"color": "white"}}>{`${word}`}</span>)
    })
...
<div className={styles.userInformation}>
    ...
    <div className={styles.tweetContent}>{tweet}
    </div>
</div>
